...
// 🔑 하드코딩된 API 키
const KAKAO_JS_KEY = "4515fe8b67a1c0e402e593f2d7bb5133"

const visitedRestaurants = [
  { id: 1, name: "마aba늘", rating: 5, address: "서울 강남 자원동 72", date: "2025-06-22", lat: 37.5665, lng: 126.9780 },
  { id: 2, name: "비놀릭", rating: 5, address: "서울 광진 자양동 72", date: "2025-06-20", lat: 37.5675, lng: 126.9790 },
  { id: 3, name: "김치찌개집", rating: 4, address: "서울 강남 테헤란로 123", date: "2025-06-18", lat: 37.5685, lng: 126.9800 },
  { id: 4, name: "파스타하우스", rating: 5, address: "서울 마포 홍대입구", date: "2025-06-15", lat: 37.5695, lng: 126.9810 },
  { id: 5, name: "일본식 라멘", rating: 4, address: "서울 강남 역삼동", date: "2025-06-12", lat: 37.5655, lng: 126.9770 },
  { id: 6, name: "중국집", rating: 3, address: "서울 서초", date: "2025-06-10", lat: 37.5645, lng: 126.9750 },
  { id: 7, name: "타코플레이스", rating: 4, address: "서울 용산 후암동", date: "2025-06-09", lat: 37.5530, lng: 126.9710 },
  { id: 8, name: "버거킹즈", rating: 3, address: "서울 송파 잠실", date: "2025-06-07", lat: 37.5115, lng: 127.0980 },
  { id: 9, name: "베이글샵", rating: 5, address: "서울 종로 인사동", date: "2025-06-05", lat: 37.5743, lng: 126.9849 },
  { id: 10, name: "케이크숍", rating: 4, address: "서울 은평 응암동", date: "2025-06-03", lat: 37.6020, lng: 126.9270 },
]
...
export default function MapPage() {
    ...
    script.onload = () => {
      window.kakao.maps.load(() => {
        ...
        const kakaoMap = new window.kakao.maps.Map(
          ...
          { center: new window.kakao.maps.LatLng(currentLocation.lat, currentLocation.lng), level: 4 }
        )
        setMap(kakaoMap)
        ...
        new window.kakao.maps.Marker({
          ...
          image: new window.kakao.maps.MarkerImage(
            "data:image/svg+xml;base64," + btoa(`<svg width="20" height="20" viewBox="0 0 20 20"
        ...
        visitedRestaurants.forEach(r => {
          const marker = new window.kakao.maps.Marker({
            ...
            image: new window.kakao.maps.MarkerImage(
              "data:image/svg+xml;base64," + btoa(`<svg width="32" height="32" viewBox="0 0 32 32"
  ...
  const filteredList = visibleList.filter(r =>
    r.name.includes(searchQuery.trim()) || r.address.includes(searchQuery.trim())
  )

  return (
    <div className="min-h-screen pb-20 bg-white">
      <div className="relative">
        <div ref={mapContainer} className="h-96 w-full rounded-b-[10px] overflow-hidden bg-gray-100" />
        ...
        <div className="absolute bottom-4 left-4 bg-white rounded-lg p-3 shadow-sm text-xs flex gap-3">
          <div className="flex items-center gap-1">
            <span className="w-4 h-4 rounded-full bg-blue-500" />현재 위치
          </div>
          <div className="flex items-center gap-1">
      ...
      <div className="px-4 py-4 space-y-3">
        {filteredList.length === 0 && (
          <p className="text-sm text-gray-500">현재 지도 범위에 음식점이 없습니다.</p>
        )}
        {filteredList.map(r => (
          <div key={r.id} className="border-b border-gray-100 py-3">
            ...
            <div className="flex items-center gap-1 text-sm text-gray-600">
              <MapPin className="w-4 h-4 text-gray-400" />{r.address}
            </div>
