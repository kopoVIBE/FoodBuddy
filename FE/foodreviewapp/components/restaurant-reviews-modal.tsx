    ...
    <div className={`fixed inset-0 flex items-center justify-center p-4 ${className}`}>
      ...
      <div className="relative w-full max-w-md bg-white rounded-2xl overflow-hidden animate-in zoom-in-95 duration-300 max-h-[85vh] flex flex-col">
        ...
        <div className="flex items-center justify-between p-4 bg-white sticky top-0 z-10">
          <h3 className="text-lg font-semibold">{restaurantName} 리뷰</h3>
          ...
          {reviews.map((review) => (
            <Card key={review.id} className="border-0 shadow-sm bg-gray-50">
              <CardContent className="p-3">
                ...
                {/* 리뷰 내용 */}
                <div className="mb-2">
                  <p className="text-[10px] leading-3 font-light text-[#333333]">{review.content}</p>
                </div>
                ...
                <div className="flex flex-wrap gap-1 mb-2">
                  {review.tags.map((tag, index) => (
                    <Badge
                      key={index}
                      variant="secondary"
                      className="text-[8px] px-2 py-0.5 bg-orange-100 text-orange-700"
                    >
                      {tag}
                    </Badge>
                  ))}
                ...
                {review.receiptImage && (
                  <div className="flex justify-end">
                    <div className="w-[60px] h-[60px] bg-gray-200 rounded-lg overflow-hidden flex-shrink-0">
                      <Image
        ...
        <div className="p-4 bg-white mb-4">
          <Button className="w-full bg-[#EB4C34] hover:bg-[#d63e2a] text-white rounded-[10px]">새 리뷰 작성하기</Button>
        </div>
      </div>
    </div>
  )
